...
- get_drag_margin(margin: int) -> float - Returns the specified `Side`'s margin.
- get_limit(margin: int) -> int - Returns the camera limit for the specified `Side`.
- get_screen_center_position() -> Vector2 - Returns the center of the screen from this camera's point of view, in global coordinates.
- get_screen_rotation() -> float - Returns the current screen rotation from this camera's point of view.
- get_target_position() -> Vector2 - Returns this camera's target position, in global coordinates.
- is_current() -> bool - Returns `true` if this Camera2D is the active camera (see `Viewport.
- make_current() - Forces this Camera2D to become the current active one.
- reset_smoothing() - Sets the camera's position immediately to its current smoothing destination.
- set_drag_margin(margin: int, drag_margin: float) - Sets the specified `Side`'s margin.
- set_limit(margin: int, limit: int) - Sets the camera limit for the specified `Side`.
