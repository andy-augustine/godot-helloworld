...
- emit_particle(xform: Transform3D, velocity: Vector3, color: Color, custom: Color, flags: int) - Emits a single particle.
- get_draw_pass_mesh(pass: int) -> Mesh - Returns the Mesh that is drawn at index `pass`.
- request_particles_process(process_time: float, process_time_residual: float = 0.0) - Requests the particles to process for extra process time during a single frame.
