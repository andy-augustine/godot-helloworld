...
**Methods:**
- distance_to_target() -> float - Returns the distance to the target position, using the agent's global position.
...
- get_current_navigation_path() -> PackedVector3Array - Returns this agent's current path from start to finish in global coordinates.
...
- get_navigation_map() -> RID - Returns the RID of the navigation map for this NavigationAgent node.
...
- get_path_length() -> float - Returns the length of the currently calculated path.
...
- is_target_reached() -> bool - Returns `true` if the agent reached the target, i.
...
- set_velocity_forced(velocity: Vector3) - Replaces the internal velocity in the collision avoidance simulation with `velocity`.
