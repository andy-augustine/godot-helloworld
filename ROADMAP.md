...
Updated 2026-04-28 (post multiplayer research crawl — decision-grade survey landed; no code changes).
...
| **Most recent ship** | **HUD overhaul + ThirdRoom endgame (Beta).** Big polish pass. AbilityStrip slots are now layered hole/raised compositions — locked = recessed dark socket, unlocked = pickup-style raised box matching what was just collected; pickup-on-collect plays a 3-revolution swirl-and-fly animation that lands the box in its slot. Inventory + ACTIVE merged into one styled panel (4 inv slots + 1 active, all 44×32, drag-drop fixed via MOUSE_FILTER_IGNORE on decorative containers). Healthbar shifted right for even top-row balance. ThirdRoom got endgame: deepened CoolantPool with tank container walls (StaticBody, ~80%-jump escape calibration), Piranha hazard (24px, 25 dmg per bite with 1.2s cooldown), and a Crown prize on the far right (head-spin scale.x = cos rotation, glow inside Visual). Crown pickup triggers HUD `play_crown_pickup` — white screen flash + 280-particle rainbow confetti from BR to TL (8-stop CONSTANT-interpolation gradient via `color_initial_ramp`, gravity arcs the pieces back down before they reach the ceiling/left wall). Player shadow rebuilt as Sprite2D with a programmatically-generated soft radial-gradient texture (56×16, quadratic alpha falloff), repositioned under visual feet, z-ordered between ground and Rig. Commits [`7961169`](https://github.com/andy-augustine/godot-helloworld/commit/7961169) → [`5d58c3d`](https://github.com/andy-augustine/godot-helloworld/commit/5d58c3d). |
| **Previous ship** | Visually distinct rooms — "Hollow Threshold" theme. Plan archived at [`plans/done/visually-distinct-rooms.md`](plans/done/visually-distinct-rooms.md). |
| **Recent research artifacts** | **Multiplayer-for-Godot decision-grade survey (2026-04-28)** → [`research/multiplayer/survey.md`](research/multiplayer/survey.md). Recommends GodotSteam + Steam Networking Sockets transport (free, Dome Keeper proof point) + `foxssake/netfox` for client-side prediction; defer dedicated servers until PvP is on the table. Verdicts on user's four hopes: Godot built-in PARTIAL, OSS TRUE, Steam TRUE, AWS GameLift FALSE (~$1,330/mo single-region, no Godot SDK). Plan archived at [`plans/done/multiplayer-research.md`](plans/done/multiplayer-research.md). Surprises captured: Hathora shuts down 2026-05-05; GodotSteam GitHub archived (now Codeberg); Photon Fusion Godot still dev-preview. Earlier intel crawl still current at [`research/tools/godot-4.6-current-intel.md`](research/tools/godot-4.6-current-intel.md). |
