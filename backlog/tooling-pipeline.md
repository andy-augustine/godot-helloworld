...
## 12. Overnight crawl — current Godot 4.6 / GDScript community knowledge — **complete (2026-04-26), refreshed (2026-06-01), refreshed (2026-07-01)**

**Status:** Shipped. Deliverable at [`research/tools/godot-4.6-current-intel.md`](../research/tools/godot-4.6-current-intel.md). Runbook preserved at [`research/crawl/PLAN.md`](../research/crawl/PLAN.md) for re-runs (re-execute via the `/refresh-godot-intel` slash command). Recommended re-scan cadence: monthly + event-driven on each Godot release; quarterly contributor/source re-rank. Last refreshed 2026-07-01 (pre-scan + 5 topic agents + synthesis; major surprise: Godot 4.7 stable shipped June 18 2026 — fixes `Input.parse_input_event` button drop, TileMapLayer-Camera2D vanish, animation hash collisions; low-risk upgrade-when-convenient).
